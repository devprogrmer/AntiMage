...
func TestExportImportFullBackupFileRoots(t *testing.T) {
	...
	service := NewService(db, "sqlite", sqliteURL(dbPath), WithFileRoots([]FileRoot{
		{ArchiveName: "etc_antimage", Path: configRoot},
		{ArchiveName: "var_lib_antimage", Path: dataRoot},
		{ArchiveName: "antimage_env", Path: envFile},
	}))
...
func TestMySQLBackupURLParsing(t *testing.T) {
	service := NewService(nil, "mysql", "mysql+pymysql://antimage:p%40ss%21%23frag%3Deq%5Cslash@127.0.0.1:3306/antimage")
...
func TestFilterMySQLDumpForDatabase(t *testing.T) {
	...
	content := strings.Join([]string{
		"-- Current Database: `antimage`",
		"/*!40000 DROP DATABASE IF EXISTS `antimage`*/;",
		"CREATE DATABASE /*!32312 IF NOT EXISTS*/ `antimage` /*!40100 DEFAULT CHARACTER SET utf8mb4 */;",
		"USE `antimage`;",
	...
	for _, forbidden := range []string{"DROP DATABASE", "CREATE DATABASE", "USE `antimage`"} {
