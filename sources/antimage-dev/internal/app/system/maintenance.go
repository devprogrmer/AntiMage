...
func BuildantimageUpdateArgs(channel string, version string) ([]string, error) {
	args := []string{"update"}
...
func resolveantimageCLI() (string, error) {
	candidates := []string{strings.TrimSpace(os.Getenv("ANTIMAGE_SCRIPT_BIN"))}
	if path, err := exec.LookPath("antimage"); err == nil {
		candidates = append(candidates, path)
	}
	candidates = append(candidates, "/usr/local/bin/antimage")
	...
	return "", MaintenanceError{Status: http.StatusServiceUnavailable, Detail: "antimage CLI was not found on this host"}
}
