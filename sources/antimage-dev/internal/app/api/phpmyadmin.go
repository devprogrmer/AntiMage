...
const phpMyAdminEmbedCookie = "antimage_pma_embed"
...
func (s *Server) handlePHPMyAdminEnable(w http.ResponseWriter, r *http.Request) {
	...
	output, err := runantimagePHPMyAdminCommand(r.Context(), "enable-phpmyadmin", "--path", path)
	if err != nil {
...
func (s *Server) handlePHPMyAdminDisable(w http.ResponseWriter, r *http.Request) {
	...
	output, err := runantimagePHPMyAdminCommand(r.Context(), "disable-phpmyadmin")
	if err != nil {
		writeError(w, http.StatusBadGateway, strings.TrimSpace(output+"\n"+err.Error()))
		return
	}
...
func ensurePHPMyAdminRuntimeConfig(credentials phpMyAdminCredentials, theme string) error {
	...
	config := "<?php\n" +
		...
		"$cfg['ExecTimeLimit'] = 0;\n" +
		themeLine
	if err := os.WriteFile("/etc/phpmyadmin/conf.d/antimage.php", []byte(config), 0o644); err != nil {
		return fmt.Errorf("write phpMyAdmin runtime config: %w", err)
	}
	if err := os.Chmod("/etc/phpmyadmin/conf.d/antimage.php", 0o644); err != nil {
...
func runantimagePHPMyAdminCommand(parent context.Context, args ...string) (string, error) {
	ctx, cancel := context.WithTimeout(parent, 10*time.Minute)
	defer cancel()
	binary := "antimage"
	if _, err := os.Stat("/usr/local/bin/antimage"); err == nil {
		binary = "/usr/local/bin/antimage"
	}
...
<body>
<div class="loading" id="loading"><div class="panel"><strong>Opening phpMyAdmin...</strong><p class="muted">antimage is signing in with the configured database account for this full-access session.</p></div></div>
<iframe id="pma-frame" src="` + escapedLoginURL + `" title="phpMyAdmin" onload="antimageRevealPHPMyAdmin()"></iframe>
<script>
` + themeCookieScript + `
function antimageRevealPHPMyAdmin(){
...
var antimagePMARevealAttempts=0;
var antimagePMARevealTimer=setInterval(function(){
  antimageRevealPHPMyAdmin();
  antimagePMARevealAttempts++;
  if (antimagePMARevealAttempts > 40) clearInterval(antimagePMARevealTimer);
}, 250);
</script>
</body>
</html>`
...
func newPHPMyAdminEmbedSecret() []byte {
	...
	return []byte(fmt.Sprintf("antimage-pma-%d", time.Now().UnixNano()))
}
