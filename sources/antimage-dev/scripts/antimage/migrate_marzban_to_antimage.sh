...
log() {
    echo -e "\e[96m[antimage-migrate]\e[0m $1"
}

warn() {
    echo -e "\e[93m[antimage-migrate]\e[0m $1"
}

error_exit() {
    echo -e "\e[91m[antimage-migrate]\e[0m $1"
    exit 1
}
...
def replace_paths(value: str) -> str:
    ...
    value = value.replace("/opt/marzban", "/opt/antimage")
    value = value.replace("Marzban", "antimage")
    return value
...
def replace_names(value: str) -> str:
    value = re.sub(r'^(\s*)marzban(\s*:)', r'\1antimage\2', value, flags=re.MULTILINE)
    value = re.sub(r'(container_name:\s*["\']?)marzban(["\']?)', r'\1antimage\2', value)
    return value
...
for line in lines:
    ...
    new_value = new_value.replace("/opt/marzban", "/opt/antimage")
...
install_antimage_cli() {
    if ! command -v curl >/dev/null 2>&1; then
        error_exit "curl is required to install the antimage CLI."
    fi
    if [ -f "/usr/local/bin/antimage" ]; then
        backup_existing_path "/usr/local/bin/antimage"
    fi
    curl -sSL "$SCRIPT_URL" | install -m 755 /dev/stdin /usr/local/bin/antimage
    warn "Old antimage CLI (if any) can now be removed manually."
    log "Installed antimage CLI to /usr/local/bin/antimage"
}
...
rerun_install_service_script() {
    ...
    systemctl enable --now antimage.service >/dev/null 2>&1 || true
    ...
    try:
        ...
        if cursor.fetchone():
            ...
            if result:
                ...
                config_str = config_str.replace("/opt/marzban", "/opt/antimage")
...
restart_antimage_panel() {
    local compose_bin="$1"
    local compose_file="$NEW_APP_DIR/docker-compose.yml"

    if command -v antimage >/dev/null 2>&1; then
        log "All migration steps completed. Handing over to 'antimage restart' as final step."
        log "From now on it is the same as running: antimage restart"
        exec antimage restart
    fi

    if [ ! -f "$compose_file" ]; then
        warn "Cannot find $compose_file to start antimage via docker compose."
        return
    fi

    log "antimage CLI not found; starting antimage stack using docker compose."
    if $compose_bin -f "$compose_file" -p "$NEW_SERVICE_NAME" up -d --remove-orphans; then
        log "antimage stack started successfully via docker compose."
    else
...
main() {
    ...
    restart_antimage_panel "$compose_bin"

    log "Migration complete. You can now manage the panel using the 'antimage' command."
    log "For example: antimage up"
}
