...
log() {
    echo -e "\e[96m[antimage-node-migrate]\e[0m $1"
}

warn() {
    echo -e "\e[93m[antimage-node-migrate]\e[0m $1"
}

error_exit() {
    echo -e "\e[91m[antimage-node-migrate]\e[0m $1"
    exit 1
}
...
install_node_cli() {
    if ! command -v curl >/dev/null 2>&1; then
        error_exit "curl is required to install the AntiMage-node CLI."
    fi
