...
ANTIMAGE_RELEASE_REPO="${ANTIMAGE_RELEASE_REPO:-devprogrmer/AntiMage}"
...
install_host_database() {
    ...
    case "$database_type" in
        mysql)
            ...
            service_name="mysql"
            config_file="/etc/mysql/mysql.conf.d/antimage.cnf"
        ;;
        mariadb)
            package_name="mariadb-server"
            service_name="mariadb"
            config_file="/etc/mysql/mariadb.conf.d/60-antimage.cnf"
        ;;
        *)
            return 0
        ;;
...
optimize_antimage_server() {
    local sysctl_file="/etc/sysctl.d/99-antimage-network.conf" available_cc=""
