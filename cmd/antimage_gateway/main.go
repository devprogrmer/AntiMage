...
func main() {
	...
	if err != nil {
		logging.Fatalf(logging.ComponentRuntime, "failed to load AntiMage API config: %v", err)
	}
	api, err := api.New(apiCfg)
	if err != nil {
		logging.Fatalf(logging.ComponentRuntime, "failed to initialize AntiMage API: %v", err)
	}
