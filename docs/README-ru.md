...
- [Введение](#введение)
  - [Почему AntiMage?](#почему-antimage)
    - [Функции](#функции)
...
AntiMage — это инструмент управления прокси-серверами, который предоставляет простой и удобный пользовательский интерфейс для управления сотнями учетных записей прокси на базе [Xray-core](https://github.com/XTLS/Xray-core) и созданный на Go и ReactJS.

## Почему AntiMage?

AntiMage удобен в использовании, многофункционален и надежен. Он позволяет создавать различные прокси для пользователей без сложной настройки. С помощью встроенного веб-интерфейса можно контролировать, изменять и ограничивать пользователей.

### Функции

- Готовый **Web UI**
- **REST API** бэкэнд
- Поддержка [**множества узлов**](#antimage-node) (для распределения инфраструктуры и масштабируемости)
...
Установите AntiMage master через бинарный установщик:
...
Установите AntiMage-node на каждом node-сервере через бинарный установщик node:
...
- Файлы AntiMage будут размещены по адресу `/opt/antimage`.
- Файл конфигурации будет размещен по адресу `/opt/antimage/.env` (см. [Конфигурация](#конфигурация)).
- Файлы с данными будут размещены по адресу `/var/lib/antimage`.
- По соображениям безопасности, панель управления AntiMage недоступна через IP-адрес. Поэтому вам необходимо получить SSL-сертификат и получить доступ к панели управления AntiMage, открыв веб-браузер и перейдя по адресу `https://YOUR_DOMAIN:8000/dashboard/` (замените YOUR_DOMAIN на ваш фактический домен).
- Вы также можете использовать перенаправление портов SSH для локального доступа к панели управления AntiMage без домена. Замените `user@serverip` на ваше фактическое имя пользователя SSH и IP-адрес сервера и выполните следующую команду:

```bash
ssh -L 8000:localhost:8000 user@serverip
```

Наконец, введите следующую ссылку в ваш браузер, чтобы получить доступ к панели управления AntiMage:
...
Далее, Вам нужно создать главного администратора для входа в панель управления AntiMage, выполнив следующую команду: 
...
Для того, чтобы увидеть справочное сообщение от скрипта AntiMage, выполните команду:
...
Если Вы хотите использовать `antimage-cli`, необходимо связать его с файлом в `$PATH`, сделать его исполняемым и установить:
...
server {
    ...
    # The '/antimage' in location regex path can changed any characters by yourself.
    #
...
| ANTIMAGE_GATEWAY_ADDR                     | Полный адрес gateway; переопределяет `UVICORN_HOST`/`UVICORN_PORT`.                                          |
| ANTIMAGE_NODE_OPERATIONS_POLL_INTERVAL    | Интервал обработки очереди node operations.                                                                   |
| ANTIMAGE_USER_LIFECYCLE_INTERVAL          | Интервал проверки lifecycle пользователей.                                                                    |
| ANTIMAGE_USER_USAGE_RESET_INTERVAL        | Интервал периодического reset usage пользователей.                                                            |
...
| ANTIMAGE_CERT_BASE                        | Базовый путь управляемых сертификатов.                                                                        |
| ANTIMAGE_CONFIG_DIR                       | Корень конфигурации, включаемый в full backup.                                                                |

# Telegram Bot

AntiMage поставляется с встроенным ботом Telegram, который может управлять сервером, создавать и удалять пользователей, а также отправлять уведомления. Этот бот можно легко включить, выполнив несколько простых шагов, и он предоставляет удобный способ взаимодействия с AntiMage без необходимости каждый раз заходить на сервер.
...
Вы можете задать адрес webhook, и AntiMage будет отправлять уведомления на этот адрес.
...
Пример запроса, отправленного AntiMage:
...
Если вы нашли AntiMage полезным и хотели бы поддержать его развитие, вы можете сделать пожертвование в одной из следующих криптовалютных сетей:
...
Мы ❤️‍🔥 участников проекта! Если вы хотите внести свой вклад, пожалуйста, ознакомьтесь с нашим [Contributing Guidelines](CONTRIBUTING.md) и не стесняйтесь отправлять запросы на исправление ошибок или сообщить о проблеме.

Проверьте [open issues](https://github.com/devprogrmer/AntiMage/issues), чтобы помочь развитию этого проекта.

<p align="center">
Спасибо всем участникам, благодаря которым AntiMage становится лучше:
</p>
<p align="center">
