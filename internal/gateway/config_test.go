...
func TestLoadConfigUsesLegacyUvicornEnv(t *testing.T) {
	...
	t.Setenv("UVICORN_SSL_CERTFILE", "/tmp/antimage/fullchain.pem")
	t.Setenv("UVICORN_SSL_KEYFILE", "/tmp/antimage/key.pem")
	...
	if cfg.TLSCertFile != "/tmp/antimage/fullchain.pem" {
		t.Fatalf("TLSCertFile=%q", cfg.TLSCertFile)
	}
	if cfg.TLSKeyFile != "/tmp/antimage/key.pem" {
...
func TestLoadConfigReadsAntiMageEnvFile(t *testing.T) {
