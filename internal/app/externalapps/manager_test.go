...
func TestMirzaBotUpdatePreservesConfigurationAndRunsMigration(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("requires Unix user tooling")
	}
...
exit 0`)
	...
	cronSecret, err := os.ReadFile(filepath.Join(root, ".antimage-cron-secret"))
	...
	if err != nil || !strings.Contains(string(commands), "php .antimage-init.php") {
...
func TestMirzaRequestSecretsAreIndependent(t *testing.T) {
	...
	cron.Header.Set("X-AntiMage-Cron-Secret", "webhook-secret")
	...
	cron.Header.Set("X-AntiMage-Cron-Secret", "cron-secret")
	if err := manager.AuthorizeMirzaRequest(cron, record, "cronbot/statusday.php"); err != nil {
		t.Fatal(err)
	}
}
...
func TestWriteMirzaCronUsesPerAppIdentityWithoutEmbeddingSecret(t *testing.T) {
	...
	record := Record{
		...
		CronConfig: filepath.Join(t.TempDir(), "antimage-php-abcdef"),
	}
	...
	if !strings.Contains(text, "rbphp_abcdef") || !strings.Contains(text, ".antimage-cron-secret") {
...
func TestWriteExternalAppPoolBoundsResources(t *testing.T) {
	record := Record{
		Root:       t.TempDir(),
		SystemUser: "rbphp_abcdef",
		Socket:     "/run/php/antimage-abcdef.sock",
		PoolConfig: filepath.Join(t.TempDir(), "antimage-abcdef.conf"),
	}
...
func TestMigrateLegacyBaseDirPreservesAndRecoversLegacyStorage(t *testing.T) {
	...
	t.Run("already renamed tree gets legacy alias", func(t *testing.T) {
		if runtime.GOOS == "windows" {
			t.Skip("requires symlink privileges that are not available in this Windows test environment")
		}
		parent := t.TempDir()
		oldBase := filepath.Join(parent, "php-apps")
		base := filepath.Join(parent, "external-apps")
...
func TestCreateExternalAppDatabaseGrantsAntiMageAccess(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("uses a POSIX shell test double")
	}
	bin := t.TempDir()
...
`
	...
	manager := &Manager{databaseURL: "mysql://antimage:secret@127.0.0.1:3306/antimage"}
	...
	for _, expected := range []string{
		...
		"GRANT ALL PRIVILEGES ON `project_db`.* TO 'antimage'@'127.0.0.1'",
		"GRANT ALL PRIVILEGES ON `project_db`.* TO 'antimage'@'localhost'",
	} {
...
func TestDownloadMirzaBotUsesLatestStableReleaseCommit(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Header.Get("User-Agent") != "AntiMage" {
