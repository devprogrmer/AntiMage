...
type pptpUsageRuntimeConfig struct {
	InboundTag string           `json:"inbound_tag"`
	Users      map[string]int64 `json:"users"`
}

type pptpPPPSession struct {
	Interface string
	PeerIP    string
}

type pptpUsageSample struct {
...
var pptpPPPInterfacePattern = regexp.MustCompile(`^ppp[0-9]+$`)

func parsePPTPPPPInterfaces(raw string) []pptpPPPSession {
	result := make([]pptpPPPSession, 0)
	...
	for _, line := range strings.Split(raw, "\n") {
		...
		if !pptpPPPInterfacePattern.MatchString(iface) {
			continue
		}
		...
		result = append(result, pptpPPPSession{
			Interface: iface,
			PeerIP:    addr.String(),
		})
	}
...
func (s *Server) activePPTPRuntimeTags() []string {
	s.mu.Lock()
	defer s.mu.Unlock()

	tags := make([]string, 0, len(s.pptpRuntimes))
	for tag := range s.pptpRuntimes {
...
func (s *Server) collectPPTPUserUsage(
	ctx context.Context,
	...
	for key, value := range s.pptpUsageBaseline {
	...
	for _, tag := range s.activePPTPRuntimeTags() {
		...
		if err != nil {
			...
			s.appendLog(
				"pptp usage config read failed for " +
					tag + ": " + err.Error(),
			)
			continue
		}
		...
		if err := json.Unmarshal(rawConfig, &cfg); err != nil {
			s.appendLog(
				"pptp usage config parse failed for " +
					tag + ": " + err.Error(),
			)
			continue
		}
		...
		for _, session := range pppSessions {
			...
			if err != nil {
				s.appendLog(
					"pptp rx counter read failed for " +
			...
			if err != nil {
				s.appendLog(
					"pptp tx counter read failed for " +
						session.Interface + ": " + err.Error(),
				)
				continue
			}
			...
			baselineKey := pptpUsageBaselineKey(
				cfg.InboundTag,
				session.Interface,
				session.PeerIP,
			)
	...
	s.pptpUsagePending = pending
	if err := s.persistPPTPUsageStateLocked(); err != nil {
		s.pptpUsagePending = nil
		return nil, err
	}

	return pptpUsageBatchProto(pending), nil
}

func (s *Server) ackPPTPUserUsage(
	_ context.Context,
	...
	if s.pptpUsageLastAckedBatchID == batchID {
		return &nodev1.AckUsageResponse{
			Acknowledged: true,
		}, nil
	}
	...
	if err := s.persistPPTPUsageStateLocked(); err != nil {
		s.pptpUsageBaseline = previousBaseline
		s.pptpUsagePending = pending
		s.pptpUsageLastAckedBatchID = previousLastAcked
		return nil, err
	}
