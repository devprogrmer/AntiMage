...
type Server struct {
	...
	liveInboundSpeedsMu  sync.RWMutex
	liveInboundSpeeds    map[string]liveInboundSpeed
...
func (s *Server) handleNodePath(w http.ResponseWriter, r *http.Request) {
	...
	case "wireguard/runtime":
		if r.Method != http.MethodGet {
			writeError(w, http.StatusMethodNotAllowed, "method not allowed")
			return
		}
		s.handleNodeWireGuardRuntime(w, r, id)
	case "vpn/runtime/wireguard", "vpn/runtime/wg", "vpn/runtime/amneziawg", "vpn/runtime/awg", "vpn/runtime/openvpn", "vpn/runtime/ov", "vpn/runtime/l2tp", "vpn/runtime/l2tp-ipsec", "vpn/runtime/pptp", "vpn/runtime/ikev2", "vpn/runtime/anyconnect", "vpn/runtime/cisco":
		if r.Method != http.MethodGet {
			writeError(w, http.StatusMethodNotAllowed, "method not allowed")
			return
		}
		protocol := strings.TrimPrefix(suffix, "vpn/runtime/")
		s.handleNodeProtocolRuntime(w, r, id, protocol)
	case "usage/daily":
		if r.Method != http.MethodGet {
			writeError(w, http.StatusMethodNotAllowed, "method not allowed")
			return
