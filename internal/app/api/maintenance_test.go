...
func (s *recordingScheduler) ScheduleWithProgress(args []string, onOutput func(string), onDone func(error)) error {
	...
	if onOutput != nil {
		onOutput("Downloading AntiMage binary 42%")
		onOutput("Restarting AntiMage service")
	}
	return nil
}
...
func TestMaintenanceInfoBinaryAndDockerMock(t *testing.T) {
	...
	if info.Panel.Mode != "binary" ||
		...
		info.Panel.Service != "antimage" ||
		info.Panel.Image != "antimage-server (binary)" ||
		info.Panel.Tag == nil ||
		*info.Panel.Tag != tag ||
...
func TestMaintenanceActionsAcceptedAndValidated(t *testing.T) {
	...
	if updateResponse.Status != "accepted" || updateResponse.Operation.Action != "update" || updateResponse.Operation.Phase != "restarting" || updateResponse.Operation.Progress == nil || *updateResponse.Operation.Progress != 100 || !updateResponse.Operation.NeedsReload {
	...
	if status.Action != "update" || status.Progress == nil || *status.Progress != 100 {
