...
func TestManagerKeepsLegacyENVCertificateSeparate(t *testing.T) {
	db, err := sql.Open("sqlite", "file:"+filepath.Join(t.TempDir(), "certificates.db"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestManagerImportsListsAndDeletesManualCertificate(t *testing.T) {
	...
	metadata["certbot_cert_name"] = "antimage-bot.example.com"
	...
	manager.run = func(_ context.Context, name string, args ...string) ([]byte, error) {
		...
		if !strings.Contains(command, " --cert-name antimage-bot.example.com ") {
