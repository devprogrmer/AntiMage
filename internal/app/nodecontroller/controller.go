...
func (c Controller) WGRuntime(ctx context.Context, nodeID int64) (WGRuntime, error) {
	return c.repo.WGRuntime(ctx, nodeID)
}

// ProtocolRuntime exposes the native runtime payload for additional protocol
...
func (c Controller) ProtocolRuntime(ctx context.Context, nodeID int64, protocol string) (any, error) {
	...
	case "l2tp", "l2tp-ipsec":
		return c.repo.L2TPRuntime(ctx, nodeID)
	case "pptp":
		return c.repo.PPTPRuntime(ctx, nodeID)
	case "ikev2":
		return c.repo.IKEv2Runtime(ctx, nodeID)
	case "anyconnect", "cisco":
		return c.repo.AnyConnectRuntime(ctx, nodeID)
	default:
		return nil, fmt.Errorf("unsupported protocol runtime %q", protocol)
	}
}
...
func (c Controller) dial(ctx context.Context, nodeID int64) (*nodeclient.Client, NodeRow, error) {
	...
	for _, address := range addresses {
		...
		if err == nil {
			hello, helloErr := client.Control().Hello(attemptCtx, &nodev1.HelloRequest{MasterId: "antimage-master"})
			if helloErr != nil {
				_ = client.Close()
				err = helloErr
			} else {
