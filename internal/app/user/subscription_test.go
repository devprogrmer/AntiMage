...
func TestSingBoxSubscriptionUsesFullTemplateAndRealNames(t *testing.T) {
	...
	settings := `{"wsSettings":{"headers":{"User-Agent":"AntiMage"}}}`
	...
	for _, raw := range config["outbounds"].([]any) {
		...
		case "vless":
			...
			if headers["User-Agent"] != "AntiMage" || headers["Host"] == "" {
...
func TestBundledSubscriptionPageTemplateRendersPanelStyleContext(t *testing.T) {
	...
	html, err := renderSubscriptionPageTemplate(template, UserDetail{
		...
		"l2tp":       []L2TPInfo{{HostName: "L2TP Edge", Server: "l2tp.example.com", Username: "alice", Password: "l2tp-password", IPSecPSK: "l2tp-psk"}},
		"pptp":       []PPTPInfo{{HostName: "PPTP Edge", Server: "pptp.example.com", Username: "alice", Password: "pptp-password"}},
	...
	for _, expected := range []string{
		...
		`class="am-credential-label" data-i18n="server"`,
		`class="am-credential-label" data-i18n="username"`,
		`class="am-credential-label" data-i18n="password"`,
		`class="am-credential-label" data-i18n="ipsecPSK"`,
		`l2tp.example.com`,
		`l2tp-psk`,
		`pptp.example.com`,
		`pptp-password`,
